...
/** Mirrors YouTubeOAuthController's connect/callback shape exactly — same double-submit
 * CSRF pattern via OAuthStateService, same error-classification style. */
@Controller('auth/google-drive')
export class GoogleDriveOAuthController {
  private readonly logger = new Logger(GoogleDriveOAuthController.name);
  ...
  connect(@CurrentUser() user: AuthenticatedUser, @Res() res: Response) {
    const { state, nonce } = this.oauthStateService.generate(user.userId, Platform.GOOGLE_DRIVE);
    ...
    return res.redirect(this.googleDriveProvider.buildAuthUrl(state));
  }
  ...
  async callback(
    ...
    try {
      ...
      const result = this.oauthStateService.verify(state, cookieNonce, Platform.GOOGLE_DRIVE);
    ...
    try {
      const tokens = await this.googleDriveProvider.exchangeCode(code);

      const grantedScopes = new Set((tokens.scope ?? '').split(' ').filter(Boolean));
      const missingScope = this.googleDriveProvider.scopes.some((s) => !grantedScopes.has(s));
      if (missingScope) {
        await this.googleDriveProvider.revoke(tokens.accessToken);
        throw new InsufficientScopeError();
      }

      const account = await this.googleDriveProvider.fetchAccount(tokens);

      await this.socialAccountsService.upsertConnection(userId, Platform.GOOGLE_DRIVE, tokens, account);

      return res.redirect(`${this.frontendUrl}/dashboard?connection=success&platform=google_drive`);
    } catch (err) {
  ...
  private classifyError(err: unknown): ConnectionErrorReason {
    ...
    this.logger.error('Unexpected error during Google Drive OAuth callback', err as Error);
    return 'unknown_error';
  }

  private redirectWithError(res: Response, reason: ConnectionErrorReason) {
    return res.redirect(
      `${this.frontendUrl}/dashboard?connection=error&reason=${encodeURIComponent(reason)}&platform=google_drive`,
    );
  }
}
