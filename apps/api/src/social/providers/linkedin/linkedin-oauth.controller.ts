...
/** Mirrors the Google OAuth controllers' connect/callback shape exactly. */
...
export class LinkedInOAuthController {
  ...
  connect(@CurrentUser() user: AuthenticatedUser, @Res() res: Response) {
    let authUrl: string;
    try {
      const { state, nonce } = this.oauthStateService.generate(user.userId, Platform.LINKEDIN);
      authUrl = this.linkedInProvider.buildAuthUrl(state);
  ...
  async callback(
    ...
    if (error) {
      this.logger.log(`LinkedIn returned OAuth error: ${error}`);
      return this.redirectWithError(res, error === 'user_cancelled_login' || error === 'access_denied' ? 'access_denied' : 'unknown_error');
    }
    ...
    try {
      ...
      const result = this.oauthStateService.verify(state, cookieNonce, Platform.LINKEDIN);
    ...
    try {
      const tokens = await this.linkedInProvider.exchangeCode(code);

      const grantedScopes = new Set((tokens.scope ?? '').split(/[ ,]/).filter(Boolean));
      const missingScope = this.linkedInProvider.scopes.some((s) => !grantedScopes.has(s));
      if (missingScope) {
        await this.linkedInProvider.revoke(tokens.accessToken);
        throw new InsufficientScopeError();
      }

      const account = await this.linkedInProvider.fetchAccount(tokens);

      await this.socialAccountsService.upsertConnection(userId, Platform.LINKEDIN, tokens, account);

      return res.redirect(`${this.frontendUrl}/dashboard?connection=success&platform=linkedin`);
    } catch (err) {
  ...
  private classifyError(err: unknown): ConnectionErrorReason {
    ...
    this.logger.error('Unexpected error during LinkedIn OAuth callback', err as Error);
    return 'unknown_error';
  }

  private redirectWithError(res: Response, reason: ConnectionErrorReason) {
    return res.redirect(
      `${this.frontendUrl}/dashboard?connection=error&reason=${encodeURIComponent(reason)}&platform=linkedin`,
    );
  }
}
