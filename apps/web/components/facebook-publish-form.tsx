...
import { ApiError, publishVideoToFacebook } from '@/lib/api';
import { scaleIn, smooth, spring } from '@/lib/motion';

const MAX_VIDEO_SIZE_BYTES = 500 * 1024 * 1024;

const ERROR_MESSAGES: Record<string, string> = {
  no_account_connected: 'No Facebook Page connected. Connect Facebook from the dashboard first.',
  reauth_required: 'Reconnect Facebook from the dashboard — permission is missing or expired.',
  insufficient_scope: 'Reconnect Facebook from the dashboard — permission is missing or expired.',
  quota_exceeded: 'Facebook rate limit reached. Please try again shortly.',
  invalid_video: 'That file was rejected by validation.',
...
export function FacebookPublishForm({ onPublished }: { onPublished?: () => void }) {
  ...
  async function handlePublish() {
    ...
    const { promise, abort } = publishVideoToFacebook({ video: file, caption, onProgress: setProgress });
    abortUploadRef.current = abort;

    try {
      // The upload progress bar reaches 100% once the bytes are sent; the response
      // itself only arrives after Facebook finishes processing the video.
      setStage('processing');
      ...
      if (err instanceof ApiError) {
        ...
        setError('Failed to publish to Facebook.');
      }
    }
  }
  ...
  return (
    <section className="rounded-xl border border-border bg-surface/60 p-6 backdrop-blur">
      <h2 className="text-base font-semibold text-ink">Upload a video</h2>

      <AnimatePresence mode="wait">
        {stage === 'published' && platformPostId ? (
          <PublishedSuccess key="success" platformPostId={platformPostId} onCreateAnother={resetForm} />
        ) : (
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between">
                <label htmlFor="fb-caption" className="block text-sm font-medium text-ink-soft">
                  Caption
                </label>
                <span className="text-xs text-ink-faint">{caption.length}/5000</span>
              </div>
              <textarea
                id="fb-caption"
                value={caption}
              ...
              {stage === 'uploading' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-white/10">
                    <motion.div className="relative h-full overflow-hidden rounded-full bg-[#1877F2]" animate={{ width: `${progress}%` }} transition={smooth}>
            ...
            {stage === 'processing' && (
              <p className="text-sm text-ink-soft">Publishing to Facebook… this can take a moment for larger files.</p>
            )}
            ...
            <div className="flex gap-3">
              <motion.button
                ...
                transition={spring}
                className="rounded-md bg-[#1877F2] px-4 py-2 text-sm font-medium text-white shadow-[0_0_20px_-6px_rgba(24,119,242,0.6)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Publish to Facebook
              </motion.button>
              {isBusy && (
...
function PublishedSuccess({ platformPostId, onCreateAnother }: { platformPostId: string; onCreateAnother: () => void }) {
  const watchUrl = `https://www.facebook.com/watch/?v=${platformPostId}`;
  ...
  return (
    <motion.div key="success" initial="hidden" animate="visible" variants={scaleIn} className="mt-4 rounded-md border border-emerald-500/20 bg-emerald-500/10 px-5 py-6 text-center">
      ...
      <p className="mt-1 text-sm text-emerald-200/80">Your video is now on your Facebook Page.</p>
      <a href={watchUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-medium text-emerald-300 underline">
        View on Facebook →
      </a>
      <motion.button
        type="button"
